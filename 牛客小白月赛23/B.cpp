...
//		n=ecnt++;
		...
		if(n==1)cout<<1<<endl;else
		{
			...
			for(auto q:p){
				...
				while(l<=r){
					int mid = (l+r)/2;
					if(che(mid,q.fi,q.se)>=q.se&&che(mid-1,q.fi,q.se)<q.se){
						ans1=mid;break;
					};
					if(che(mid,q.fi,q.se)<q.se)
					{
						l=mid+1;
					}
					else{
...
//					ans1=mid;
//					cout<<mid<<" "<<che(mid,q.fi,q.se)<<endl;
				}			
//				cout<<ans1<<endl;
				ans=max(ans,q.fi*ans1);
			}
			cout<<ans<<endl;	
		}
